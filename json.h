...
#define json_panic(fmt, ...) \
  do { fprintf(stderr, "[ERROR] " fmt "\n" VA_ARGS(__VA_ARGS__)); return false; } while(false)
...
typedef enum JsonTokenType {
  ...
  JSON_OBJECT_COLON, JSON_COMMA,
  JSON_STRING,
  JSON_NUMBER,
  JSON_NULL,
  JSON_BOOL,
  JSON_ERROR // something unparseable
} JsonTokenType;
...
/* Read next token */
JsonToken json_token(char **at);
const char *json_token_name(JsonTokenType type);
...
#define json_object(in, body)                                                  \
  ...
  while (json_at(_json_obj) != '}') {                                          \
    ...
    json_expect_consume(_json_obj, ':');                                       \
    body json_panic("Unhandled JSON field: %s", _json_field);                  \
  }                                                                            \
  json_expect_consume(_json_obj, '}');                                         \
  in = _json_obj;


#define json_array(in, toptr, sizeptr, type, parser)                           \
  char **_json_arr = in;                                                       \
  bool _json_first = true;                                                     \
  size_t _json_capacity = 8;                                                   \
  *(sizeptr) = 0;                                                              \
  *(toptr) = calloc(_json_capacity, sizeof(type));                    \
  if (!*(toptr)) {                                                             \
    json_panic("Can't allocate initial space for array of type: %s", #type);   \
  }                                                                            \
  ...
  while (json_at(_json_arr) != ']') {                                          \
    ...
    if (*(sizeptr) == _json_capacity) {                                        \
      _json_capacity *= 1.62;                                                  \
      *(toptr) = realloc(*(toptr), _json_capacity * sizeof(type));             \
      if (!*(toptr)) {                                                         \
        json_panic("Can't allocate space for array of type: %s", #type);       \
      }                                                                        \
    }                                                                          \
    (*(toptr))[*(sizeptr)] = _json_arr_val;                                    \
    *sizeptr += 1;                                                             \
  }                                                                            \
  json_expect_consume(_json_arr, ']');                                         \
  in = _json_arr;
...
bool json_double(char **pos, double *value) {
  ...
  if(*at == '.') {
    ...
    *pos = at;
    double f = intval >= 0 ? 1.0 : -1.0;
    *value = (double) intval + (f * frac/fr);
  } else {
    // integer part only
    *value = (double) intval;
  }
...
static Str parse_string(char *at) {
  ...
  return (Str) { true, at, (size_t) (end-at), end+1 };
...
JsonToken json_token(char **at) {
  ...
  bool skip = false;
  switch(**at) {
  case 0: type = JSON_END; break;
  case '[': type = JSON_ARRAY_START; break;
  case ']': type = JSON_ARRAY_END; break;
  case '{': type = JSON_OBJECT_START; break;
  case '}': type = JSON_OBJECT_END; break;
  case ':': type = JSON_OBJECT_COLON; break;
  case ',': type = JSON_COMMA; break;
  case '"': type = JSON_STRING; skip = true; break;
  case 'n': type = JSON_NULL; skip = true; break;
  case 't': case 'f': type = JSON_BOOL; skip = true; break;
  case '0': case '1': case '2': case '3': case '4':
  case '5': case '6': case '7': case '8': case '9':
  case '+': case '-':
    type = JSON_NUMBER; skip = true; break;
  default:
    type = JSON_ERROR;
  }
  if(skip) {
  ...
  //printf("TOKEN type %s :: (%d) \"%.*s\"\n", json_token_name(type), (int)len, (int)len, start);
  return (JsonToken) {.type = type, .start = start, .len = len };
}

const char *json_token_name(JsonTokenType t) {
  if(t < 0 || t > JSON_ERROR) return "UNKOWN";
  return json_token_names[t];
}
